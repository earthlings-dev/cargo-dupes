...
fn retain_non_test_units(
    analyzer: &dyn LanguageAnalyzer,
    exclude_tests: bool,
    test_ranges: &mut TestRanges,
...
pub fn analyze_units_with_generic(
    ...
    let sub_groups = compute_sub_ast_groups(
        units,
        explicit_sub_units,
        config,
        &mut suppressed_groups,
        &mut rule_unit_counts,
    );
    let duplicate_coverage = DuplicateCoverage::from_matched_groups(
        &ast_groups,
        sub_groups_have_precise_spans.then_some(&sub_groups),
    );
    ...
    let all_member_fingerprint_sets =
        collect_member_fingerprint_sets(&ast_groups, &sub_groups, &generic_groups);

    let ignore_file = ignore::load_ignore_file(&config.root);
    let visible_before_ignore = ast_groups.len()
        + sub_groups.len()
        + generic_groups.len()
        + generic_groups.suppressed.len();
    ...
    let ignored_group_count = visible_before_ignore
        - (ast_groups.len()
            + sub_groups.len()
            + generic_groups.len()
            + generic_groups.suppressed.len());
...
fn apply_coverage_notes(
    ast_groups: &mut MatchedGroups,
    sub_groups: Option<&mut MatchedGroups>,
    notes: Vec<CoverageNoteSource>,
) {
    use std::collections::BTreeMap;
    let mut counts: BTreeMap<(usize, DetectionDimension, grouper::MatchKind), usize> =
        BTreeMap::new();
    for note in notes {
        *counts
            .entry((note.coverer, note.dimension, note.match_kind))
            .or_default() += 1;
    }
    if counts.is_empty() {
        return;
    }
    let mut coverers: Vec<&mut DuplicateGroup> = ast_groups
        .exact
        .iter_mut()
        .chain(ast_groups.near.iter_mut())
        .collect();
    if let Some(sub_groups) = sub_groups {
        coverers.extend(
            sub_groups
                .exact
                .iter_mut()
                .chain(sub_groups.near.iter_mut()),
        );
    }
...
fn partition_suppressed(
    ...
    let (hidden, visible): (Vec<_>, Vec<_>) = groups.into_iter().partition(|group| {
        group
            .members
            .iter()
            .all(|member| member.suppressed.is_some())
    });
...
fn bump(
    counts: &mut std::collections::BTreeMap<suppression::RuleId, usize>,
...
fn apply_suppression_stats(
    ...
    let mut rule_group_counts: std::collections::BTreeMap<RuleId, usize> =
...
fn partition_matched(
    mut groups: MatchedGroups,
    suppressed: &mut Vec<DuplicateGroup>,
) -> MatchedGroups {
...
fn compute_sub_ast_groups(
    ...
    let mut groups = compute_matched_groups(
        &sub_units,
...
fn apply_chain_coverage(groups: &mut MatchedGroups, policy: &suppression::SuppressionPolicy) {
    ...
    let duplicated_chains: HashSet<Fingerprint> =
        grouper::member_fingerprints_iter(groups.exact.iter().filter(|group| {
            group
                .members
                .first()
    ...
    for group in groups.exact.iter_mut().chain(groups.near.iter_mut()) {
        let chain_covered = !group.members.is_empty()
            && group.members.iter().all(|member| {
                member
                    .parent_chain
                    .is_some_and(|chain| duplicated_chains.contains(&chain))
            });
        if chain_covered {
...
fn dedupe_same_region_sub_units(mut units: Vec<CodeUnit>) -> Vec<CodeUnit> {
    ...
    for unit in units {
        let redundant = kept.iter().any(|existing| {
            existing.file == unit.file
                && existing.fingerprint == unit.fingerprint
                && unit_ranges_overlap(existing, &unit)
        });
        if !redundant {
            kept.push(unit);
        }
...
fn compute_generic_groups(
    ...
    token_normalized.exact =
        suppress_overlapping_groups(token_normalized.exact, policy, &mut suppressed);
    token_normalized.near =
        suppress_overlapping_groups(token_normalized.near, policy, &mut suppressed);
    token_normalized.exact = duplicate_coverage.split_substantially_covered(
        token_normalized.exact,
        policy,
        &mut suppressed,
        notes,
    );
    token_normalized.near = duplicate_coverage.split_substantially_covered(
        token_normalized.near,
        policy,
        &mut suppressed,
        notes,
    ...
    let token_raw_exact = duplicate_coverage.split_substantially_covered(
        token_raw_exact,
        policy,
        &mut suppressed,
        notes,
    ...
    let line_exact =
        duplicate_coverage.split_substantially_covered(line_exact, policy, &mut suppressed, notes);
...
fn group_start_key(group: &DuplicateGroup) -> Option<(&PathBuf, usize)> {
    group
        .members
        .first()
        .map(|member| (&member.file, member.line_start))
}

/// Try to merge `next` into `current` as a shifted continuation.
fn merge_window_group_pair(
...
impl DuplicateCoverage {
    fn from_matched_groups(
        ast_groups: &MatchedGroups,
        precise_sub_groups: Option<&MatchedGroups>,
    ) -> Self {
    ...
    fn covering_group_index(&self, group: &DuplicateGroup) -> Option<usize> {
        self.groups.iter().position(|coverage| {
            group
                .members
                .iter()
                .all(|member| coverage.covers_unit(member))
        })
    }
}
...
impl GroupCoverage {
    ...
    fn covered_ratio(&self, unit: &CodeUnit) -> f64 {
        ...
        let mut intersections: Vec<LineRange> = ranges
            .iter()
            .filter_map(|range| range.intersection(unit_range))
            .collect();
...
fn group_is_covered_by_group(
    group: &DuplicateGroup,
    kept_group: &DuplicateGroup,
    min_overlap_ratio: f64,
) -> bool {
    group.dimension == kept_group.dimension
        && group.members.iter().all(|member| {
            kept_group.members.iter().any(|kept_member| {
                member.file == kept_member.file
                    && line_overlap_ratio(member, kept_member) >= min_overlap_ratio
            })
        })
}
...
mod tests {
    ...
    use crate::node::{BinOpKind, LiteralKind, NodeKind, NormalizedNode, PlaceholderKind};
    ...
    fn paired_function_units(
        first: &Path,
        second: &Path,
        first_body: NormalizedNode,
        second_body: NormalizedNode,
    ) -> Vec<CodeUnit> {
        vec![
    ...
    fn generic_groups_covered_by_ast_duplicates_are_suppressed() {
        ...
        let result =
            analyze_units_with_generic(&units, &[], &[], &[], &line_units, Vec::new(), &config)
                .unwrap();
    ...
    fn ignore_filtering_keeps_group_fingerprints_live_pre_policy() {
        ...
        let baseline =
            analyze_units_with_generic(&units, &[], &[], &[], &[], Vec::new(), &config).unwrap();
    ...
    fn member_fingerprint_ignores_keep_member_sets_live_pre_policy() {
        ...
        let first = tmp.path().join("first.rs");
        let second = tmp.path().join("second.rs");
        let units = paired_function_units(
            &first,
            &second,
            test_body("first near body"),
            test_body("second near body"),
        );
        ...
        let member_fingerprints: Vec<_> = near_group
            .members
            .iter()
            .map(|member| member.fingerprint.to_hex())
            .collect();
        let expected_member_set: std::collections::HashSet<_> = near_group
            .members
            .iter()
            .map(|member| member.fingerprint)
            .collect();
        ...
        assert!(
            result
                .all_member_fingerprint_sets
                .iter()
                .any(|set| set == &expected_member_set),
...
";
    ...
    fn setter_body() -> NormalizedNode {
        ...
        let var =
            |index| NormalizedNode::leaf(NodeKind::Placeholder(PlaceholderKind::Variable, index));
    ...
    fn trivial_setter_pairs_group_as_suppressed_with_their_rule() {
        ...
        assert_eq!(
            group.suppressed,
            Some(crate::suppression::RuleId::AstSetterReturningSelf)
        );
    ...
    fn mixed_groups_stay_visible_with_tagged_members() {
        ...
        let tagged: Vec<_> = group
            .members
            .iter()
            .filter(|member| member.suppressed.is_some())
            .collect();
...
";
        ...
        let result =
            analyze_units_with_generic(&units, &[], &[], &[], &all_line_units, Vec::new(), &config)
                .unwrap();

        assert_eq!(result.exact_groups.len(), 1, "the AST pair stays visible");
        assert!(
            result.line_exact_groups.is_empty(),
            "the covered line group is suppressed"
        );
        let covered = result
            .suppressed_groups
            .iter()
...
";
        ...
        assert_eq!(
            group.suppressed,
            Some(crate::suppression::RuleId::LineChainTail)
        );
        assert_eq!(group.members.len(), 2);
        assert!(result.all_fingerprints.contains(&group.fingerprint));
    }
...
";
        ...
        assert_eq!(
            plain.line_exact_groups[0].fingerprint,
            shifted.line_exact_groups[0].fingerprint,
        );
    }
    ...
    fn concept_level_line_group_wins_over_fragment_with_more_members() {
        ...
        let concept = test_group(
            DetectionDimension::Line,
            "concept",
            vec![make_unit(&first, 10, 21), make_unit(&second, 30, 41)],
        );
        let fragment = test_group(
            DetectionDimension::Line,
            "fragment",
            vec![
        ...
        let covered_fragment = test_group(
            DetectionDimension::Line,
            "covered",
            vec![make_unit(&first, 12, 16), make_unit(&second, 32, 36)],
        );
        let concept = test_group(
            DetectionDimension::Line,
            "concept",
        ...
        assert_eq!(
            ...
            Some(crate::suppression::RuleId::GroupOverlapContained)
        );
    }
...
",
    ...
    fn fallback_sub_ast_parent_spans_do_not_suppress_generic_groups() {
        ...
        let config = Config {
            ...
            sub_function: true,
            min_sub_nodes: 1,
            enabled_dimensions: BTreeSet::from([
                DetectionDimension::SubAst,
                DetectionDimension::Line,
            ]),
            ..Config::default()
