...
pub fn load_ignore_file(root: &Path) -> IgnoreFile {
  ...
  std::fs::read_to_string(&path).map_or_else(|_| IgnoreFile::default(), |content| toml::from_str(&content).unwrap_or_default())
}

/// Save the ignore file to disk.
pub fn save_ignore_file(root: &Path, ignore_file: &IgnoreFile) -> std::io::Result<()> {
  let path = ignore_file_path(root);
  let content = toml::to_string_pretty(ignore_file).map_err(|e| std::io::Error::other(format!("Failed to serialize ignore file: {e}")))?;
  std::fs::write(path, content)
}

/// Add an ignore entry for a fingerprint.
pub fn add_ignore(ignore_file: &mut IgnoreFile, fingerprint: &Fingerprint, reason: Option<String>, members: Vec<String>) {
...
#[must_use]
pub fn filter_ignored(groups: Vec<DuplicateGroup>, ignore_file: &IgnoreFile) -> Vec<DuplicateGroup> {
  groups
    .into_iter()
...
fn entry_is_live(entry: &IgnoreEntry, live_fingerprints: &HashSet<Fingerprint>, live_member_sets: &[HashSet<Fingerprint>]) -> bool {
...
mod tests {
  ...
  fn roundtrip_save_and_load() {
    ...
    add_ignore(&mut ignore, &fp, Some("test reason".to_string()), vec![
      "foo".to_string(),
      "bar".to_string(),
    ]);
  ...
  fn entry_with_member_fingerprints(seeds: &[&str]) -> IgnoreEntry {
    IgnoreEntry {
      ...
      reason:              None,
      members:             Vec::new(),
      member_fingerprints: seeds.iter().map(|seed| member_unit(seed).fingerprint.to_hex()).collect(),
    }
  }
  ...
  fn member_subset_matching_survives_membership_drift() {
    ...
    let drifted_group = DuplicateGroup {
      ...
      members:     vec![member_unit("alpha"), member_unit("bravo"), member_unit("charlie")],
      similarity:  0.92,
    };
  ...
  fn member_subset_matching_resurfaces_edited_members() {
    ...
    assert_eq!(find_stale_entries(&ignore, &HashSet::new(), &[member_set]).len(), 1);
  }
