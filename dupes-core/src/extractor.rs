//! Sub-function extraction and trivial-shape classification: walks
//! normalized bodies for branch/arm/loop/closure sub-units (if-chains
...
#[must_use]
pub fn classify_sub_unit(node: &NormalizedNode, policy: &crate::suppression::SuppressionPolicy) -> Option<crate::suppression::RuleId> {
...
fn add_if_branches(
  ...
  let mut add = |branch: &NormalizedNode, label: &str| {
    if try_add(branch, CodeUnitKind::IfBranch, label, min_node_count, results) {
...
fn extract_if_chains(node: &NormalizedNode, min_node_count: usize, results: &mut Vec<SubUnit>) -> bool {
  ...
  let mut owning_chain: std::collections::HashMap<usize, crate::fingerprint::Fingerprint> = std::collections::HashMap::new();
  for run in consecutive_runs(&chain_members) {
    let chain = NormalizedNode::with_children(
      NodeKind::Block,
      run.iter().map(|&idx| statement_if(&node.children[idx]).clone()).collect(),
    );
    let description = format!("if chain ({} branches)", run.len());
    if try_add(&chain, CodeUnitKind::IfChain, &description, min_node_count, results) {
      let chain_fp = crate::fingerprint::Fingerprint::from_node(&results.last().expect("pushed").node);
      for &idx in run {
...
fn statement_if(node: &NormalizedNode) -> &NormalizedNode {
  if matches!(node.kind, NodeKind::Semi) && node.children.len() == 1 && matches!(node.children[0].kind, NodeKind::If) {
    &node.children[0]
  } else {
    node
  }
...
fn try_add(node: &NormalizedNode, kind: CodeUnitKind, description: &str, min_node_count: usize, results: &mut Vec<SubUnit>) -> bool {
...
fn has_reportable_structure(node: &NormalizedNode) -> bool {
  match &node.kind {
    ...
    NodeKind::BinaryOp(op) => is_reportable_binary_op(op) || node.children.iter().any(has_reportable_structure),
    NodeKind::UnaryOp(_)
    | NodeKind::Block
    | NodeKind::Paren
    | NodeKind::Semi
...
fn low_information_rule(node: &NormalizedNode) -> Option<crate::suppression::RuleId> {
  ...
  match &node.kind {
    ...
    NodeKind::MacroCall {
      name,
    } => (is_message_only_macro(name) && children_are_simple_values_or_projections(node)).then_some(RuleId::SubMessageOnlyMacro),
    ...
    NodeKind::Block | NodeKind::Paren | NodeKind::Semi | NodeKind::Try if node.children.len() == 1 => {
...
fn returns_empty_default(node: &NormalizedNode) -> bool {
  node.children.iter().all(|child| match &child.kind {
    ...
    NodeKind::Call => child.children.len() == 1 && is_simple_value_or_projection(&child.children[0]),
...
fn is_simple_value_or_projection(node: &NormalizedNode) -> bool {
  match &node.kind {
    ...
    NodeKind::Block | NodeKind::Semi if node.children.len() == 1 => is_simple_value_or_projection(&node.children[0]),
    _ => false,
  }
}

fn peel_transparent_single_child(mut node: &NormalizedNode) -> &NormalizedNode {
  while matches!(node.kind, NodeKind::Block | NodeKind::Paren | NodeKind::Semi) && node.children.len() == 1 {
...
#[must_use]
pub fn classify_closure_body(body: &NormalizedNode, policy: &crate::suppression::SuppressionPolicy) -> Option<crate::suppression::RuleId> {
...
fn is_setter_returning_self(node: &NormalizedNode) -> bool {
  ...
  // value-plumbing test.
  match mutation.kind {
    NodeKind::Assign | NodeKind::MethodCall => mutation.children.iter().all(is_value_plumbing_expr),
    _ => false,
  }
}
...
fn is_trivial_boolean_projection(node: &NormalizedNode) -> bool {
  match &node.kind {
    NodeKind::BinaryOp(op) if !is_reportable_binary_op(op) => node
      .children
      .iter()
...
mod tests {
  ...
  fn reportable_sub_unit_rejects_simple_predicates() {
    ...
    assert_eq!(classify_sub_unit(&predicate, &policy()), Some(RuleId::SubNoStructure));
    assert_eq!(classify_sub_unit(&block(vec![predicate]), &policy()), Some(RuleId::SubNoStructure));
  }
  ...
  fn direct_extraction_still_respects_only_node_threshold() {
    ...
    assert_eq!(classify_sub_unit(&sub_units[0].node, &policy()), Some(RuleId::SubNoStructure));
  }
  ...
  fn reportable_sub_unit_rejects_forwarding_call_bodies() {
    ...
    assert_eq!(classify_sub_unit(&forwarding, &policy()), Some(RuleId::SubValuePlumbing));
    assert_eq!(
  ...
  fn guard_returns_of_empty_defaults_are_not_reported() {
    ...
    assert_eq!(classify_sub_unit(&guard, &policy()), Some(RuleId::SubEmptyDefaultReturn));
  }
  ...
  fn computed_returns_stay_reported() {
    ...
    assert_eq!(classify_sub_unit(&block(vec![semi(computed)]), &policy()), None);
  }
  ...
  fn error_returns_with_payloads_stay_reported() {
    ...
    assert_eq!(classify_sub_unit(&block(vec![semi(return_err)]), &policy()), None);
  }
  ...
  fn plumbing_dispatch_return_is_tagged_value_plumbing() {
    ...
    assert_eq!(classify_sub_unit(&body, &policy()), Some(RuleId::SubValuePlumbing));
  }
  ...
  fn return_with_computed_argument_stays_reportable() {
    ...
    let dispatch = call(vec![NormalizedNode::leaf(NodeKind::Path), computed_arg, variable(2)]);
  ...
  fn message_only_macro_branches_are_not_reported() {
    ...
    assert_eq!(classify_sub_unit(&branch, &policy()), Some(RuleId::SubMessageOnlyMacro));
  }
  ...
  fn assertion_macro_branches_stay_reported() {
    ...
    assert_eq!(classify_sub_unit(&block(vec![semi(oracle)]), &policy()), None);
  }
  ...
  fn panic_macro_branches_stay_reported() {
    ...
    assert_eq!(classify_sub_unit(&block(vec![semi(panic)]), &policy()), None);
  }
  ...
  fn constructor_dispatch_arms_are_not_reported() {
    ...
    assert_eq!(classify_sub_unit(&dispatch, &policy()), Some(RuleId::SubValuePlumbing));
  }
  ...
  fn constructor_assembly_arms_are_not_reported() {
    ...
    assert_eq!(classify_sub_unit(&assembly, &policy()), Some(RuleId::SubValuePlumbing));
  }
  ...
  fn projection_only_method_predicates_are_not_reported() {
    ...
    assert_eq!(classify_sub_unit(&any, &policy()), Some(RuleId::SubValuePlumbing));
  }
  ...
  fn closure_comparator_adapters_are_not_reportable() {
    ...
    assert_eq!(classify_closure_body(&cmp, &policy()), Some(RuleId::AstComparatorAdapter));
  }
  ...
  fn closures_with_callback_pipelines_stay_reportable() {
    ...
    assert_eq!(classify_closure_body(&block(vec![chain.clone()]), &policy()), None);
  ...
  fn consecutive_setter_ifs_extract_as_one_chain() {
    ...
    assert_eq!(branches.len(), 3, "chained branches stay extracted, linked to their owning chain");
  ...
  fn single_if_statement_still_extracts_branch_not_chain() {
    ...
    assert!(sub_units.iter().any(|unit| unit.kind == CodeUnitKind::IfBranch));
    assert!(sub_units.iter().all(|unit| unit.kind != CodeUnitKind::IfChain));
  }
  ...
  fn chain_members_still_extract_nested_structures() {
    ...
    assert!(sub_units.iter().any(|unit| unit.kind == CodeUnitKind::IfChain));
    assert!(
