...
mod tests {
  ...
  fn different_child_counts_uses_shared_prefix() {
    ...
    let b = NormalizedNode::with_children(NodeKind::Block, vec![int_lit(), int_lit(), int_lit(), var(0), var(1)]);
  ...
  fn same_discriminant_different_data_no_self_match() {
    ...
    let a = NormalizedNode::with_children(NodeKind::BinaryOp(BinOpKind::Add), vec![var(0), int_lit()]);
    let b = NormalizedNode::with_children(NodeKind::BinaryOp(BinOpKind::Sub), vec![var(0), int_lit()]);
    let score = similarity_score(&a, &b);
