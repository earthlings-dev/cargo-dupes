...
fn line_windows(
    ...
    let normalized: Vec<(usize, String)> = source
        ...
        .filter_map(|(idx, line)| {
            ...
            let trimmed = without_block_comments
                .split_whitespace()
                .collect::<Vec<_>>()
                .join(" ");
            if trimmed.is_empty() {
...
fn segments_form_one_stanza_block(prev: &[(usize, String)], next: &[(usize, String)]) -> bool {
    ...
    *next_start == prev_end + 2
        && segment_is_declaration_stanza(prev)
...
fn segment_is_declaration_stanza(slice: &[(usize, String)]) -> bool {
    ...
    let has_header = rows
        .first()
        .is_some_and(|(_, line)| line_is_type_header(line));
    if has_header {
        rows = &rows[1..];
    }
    ...
    rows.iter().all(|(_, line)| line_is_stanza_shaped(line))
        && (has_header || rows.iter().any(|(_, line)| line_is_structured_data(line)))
}
...
/// stays visible).
fn line_window_is_declaration_signature_prefix(
...
fn token_window_is_chain_tail(slice: &[Token]) -> bool {
    ...
    let chain_tail_lines = lines
        .iter()
        .filter(|line| line_starts_with(line, is_chain_tail_start))
        .count();
...
fn token_window_is_signature_prefix(slice: &[Token]) -> bool {
    ...
    if !slice
        .first()
        .is_some_and(|token| token.raw == "/" || token.raw == "#")
    {
        return false;
    }
    ...
    let scaffold_len = slice
        .iter()
        .position(|token| token.raw == "{")
        .unwrap_or(slice.len());
...
fn token_window_is_declaration_scaffold(slice: &[Token]) -> bool {
    ...
    let field_lines = window_token_lines(slice)
        .iter()
        .filter(|line| line_is_field_like(line))
        .count();
...
fn line_is_field_like(line: &[Token]) -> bool {
    line.first()
        .is_some_and(|token| token.normalized == "IDENT")
...
const BEHAVIOR_KEYWORDS: &[&str] = &[
    "async", "await", "break", "class", "const", "continue", "def", "else", "enum", "fn", "for",
    "if", "impl", "let", "loop", "match", "return", "static", "struct", "trait", "type", "where",
    "while", "yield",
];
...
fn line_window_is_declaration_stanza(slice: &[(usize, String)]) -> bool {
    let structured_lines = slice
        .iter()
        .filter(|(_, line)| line_is_structured_data(line))
        .count();
    structured_lines >= 2
        && unique_terms_in_lines(slice) >= 4
        && slice.iter().all(|(_, line)| line_is_stanza_shaped(line))
}
...
fn line_window_scores_low_signal(slice: &[(usize, String)]) -> bool {
    let meaningful_lines = slice
        .iter()
        .filter(|(_, line)| is_meaningful_line(line))
        .count();
    if meaningful_lines < 2 {
        return true;
    }

    let behavior_lines = slice
        .iter()
        .filter(|(_, line)| line_has_behavior(line))
        .count();
    ...
    !((behavior_lines > 0 || prose_lines >= 2 || structured_lines >= 2)
        && unique_terms_in_lines(slice) >= 4)
}

fn line_window_is_import_or_module_scaffold(slice: &[(usize, String)]) -> bool {
    let import_or_module_lines = slice
        .iter()
        .filter(|(_, line)| line_is_import_or_module_scaffold(line))
        .count();
...
fn line_window_is_chain_tail(slice: &[(usize, String)]) -> bool {
    let chain_tail_lines = slice
        .iter()
        .filter(|(_, line)| line_is_chain_tail(line))
        .count();
    chain_tail_lines >= 2
        && chain_tail_lines * 2 >= slice.len()
        && !slice.iter().any(|(_, line)| line_has_behavior(line))
}

fn is_meaningful_line(line: &str) -> bool {
    !line_is_low_info(line)
        && (line_has_behavior(line) || line_is_prose(line) || line_is_structured_data(line))
}
...
fn contains_word(value: &str, words: &[&str]) -> bool {
    words
        .iter()
        .any(|word| wordish_parts(value).any(|part| part == *word))
}
...
mod tests {
    ...
    fn rust_ticks_lex_char_literals_and_leave_lifetimes_as_punctuation() {
        let rust = QuoteProfile { rust_ticks: true };
...
";
        for (min_tokens, source) in [
            (40, match_table_prefix),
            (30, documented_signature),
            (20, declaration_scaffold),
        ] {
            let config = Config {
                token_min_tokens: min_tokens,
                token_min_lines: 2,
            ...
            assert!(
                units
                    .normalized_tokens
                    .iter()
                    .all(|unit| unit.suppressed.is_some()),
...
";
        assert_eq!(
            attribute(match_table_prefix, 40),
            Some(RuleId::TokenMatchTablePrefix)
        );
        let documented_signature = "\
/// Normalize one construct from the parse tree rows.
/// Produces a normalized node for the duplicate pipeline.
...
";
        assert_eq!(
            attribute(documented_signature, 30),
...
";
        ...
        assert_eq!(
            attribute(declaration_scaffold, 35),
...
";
        ...
        for (source, min_tokens, label) in [
            (
                block_arms,
                30,
                "match arms with block bodies carry behavior",
            ),
            (
                declaration_with_impl,
                25,
                "declaration-adjacent runtime behavior stays eligible",
            ),
            (
                complete_chain,
                30,
                "complete callback chains stay eligible as token windows",
            ),
        ] {
            assert!(
                !normalized_token_windows_for(source, min_tokens).is_empty(),
                "{label}"
            );
        }
    }
...
";
    ...
    fn normalized_tokens_ignore_identifier_names() {
        let first = tokenize(
            "fn alpha(x: i32) { let beta = x + 1; }",
            QuoteProfile::default(),
        );
        let second = tokenize(
            "fn gamma(y: i32) { let delta = y + 1; }",
            QuoteProfile::default(),
        );
...
";
    ...
    fn stanza_segment_accepts_trailing_close_brace_row() {
        ...
        let segment = numbered_lines(
            10,
            &["/// Final option.", "#[arg(long)]", "pub omega: bool,", "}"],
        );
        assert!(segment_is_declaration_stanza(&segment));
    }
    ...
    fn stanza_block_merges_final_stanza_before_close() {
        ...
        let first = numbered_lines(
            1,
            &["/// First option.", "#[arg(long)]", "pub alpha: usize,"],
        );
        let second = numbered_lines(
            5,
            &["/// Final option.", "#[arg(long)]", "pub omega: bool,", "}"],
        );
        let merged = coalesce_stanza_segments(vec![first.as_slice(), second.as_slice()]);
...
";
        ...
        assert!(
            units
                .lines
                .iter()
                .all(|unit| unit.line_end != 3 && unit.line_start != 6),
        ...
        assert!(
            units
                .lines
                .iter()
                .any(|unit| (unit.line_start, unit.line_end) == (3, 5)),
...
";
        ...
        let original_fps: Vec<_> = original
            .normalized_tokens
            .iter()
            .map(|u| u.fingerprint)
            .collect();
        ...
        assert!(
            moved
                .normalized_tokens
                .iter()
                .any(|u| original_fps.contains(&u.fingerprint)),
...
";
    ...
    fn strip_block_comments_keeps_quoted_and_prose_markers() {
        let mut in_comment = false;
        assert_eq!(
            strip_block_comments("rest.find(\"/*\")", &mut in_comment),
            "rest.find(\"/*\")"
        );
        assert!(!in_comment);
        assert_eq!(
        ...
        assert!(!in_comment);
        assert_eq!(
            strip_block_comments("// prose /* stays", &mut in_comment),
            "// prose /* stays"
        );
        assert!(!in_comment);
        assert_eq!(
            strip_block_comments("# python /* stays", &mut in_comment),
            "# python /* stays"
        );
        assert!(!in_comment);
        assert_eq!(
