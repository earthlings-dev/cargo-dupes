...
pub(crate) fn compare_group_size_desc_then_fingerprint(a: &DuplicateGroup, b: &DuplicateGroup) -> std::cmp::Ordering {
  b.members
    .len()
    .cmp(&a.members.len())
...
#[must_use]
pub fn find_near_duplicates(units: &[CodeUnit], threshold: f64, exact_fingerprints: &[Fingerprint]) -> Vec<DuplicateGroup> {
...
pub fn find_near_duplicates_for(
  ...
  let exact_set: std::collections::HashSet<Fingerprint> = exact_fingerprints.iter().copied().collect();

  // Filter out units that are already in exact duplicate groups
  let candidates: Vec<&CodeUnit> = units.iter().filter(|u| !exact_set.contains(&u.fingerprint)).collect();
  ...
  for unit in &candidates {
    ...
    buckets.entry((unit.kind.clone(), size_bucket)).or_default().push(unit);
  }
  ...
  let mut result: Vec<DuplicateGroup> = group_map
    ...
    .map(|member_indices| {
      ...
      let members: Vec<CodeUnit> = member_indices.iter().map(|&i| candidates[i].clone()).collect();
      ...
      DuplicateGroup {
        ...
        members,
        similarity: if min_score.is_infinite() { threshold } else { min_score },
      }
    })
    .collect();
...
pub fn compute_stats(units: &[CodeUnit], exact_groups: &[DuplicateGroup], near_groups: &[DuplicateGroup]) -> DuplicationStats {
...
pub fn with_generic_stats(
  ...
  stats.token_normalized_exact_units = token_normalized_exact_groups.iter().map(|g| g.members.len()).sum();
  stats.token_normalized_near_groups = token_normalized_near_groups.len();
  stats.token_normalized_near_units = token_normalized_near_groups.iter().map(|g| g.members.len()).sum();
