...
impl Reporter for JsonReporter {
  fn report_full(&self, result: &AnalysisResult, writer: &mut dyn io::Write) -> io::Result<()> {
    let report = JsonReport {
      ...
      groups:            result.groups().map(|group| self.to_json_group(group)).collect(),
      suppressed_groups: if self.options.show_suppressed {
        result.suppressed_groups.iter().map(|group| self.to_json_group(group)).collect()
      } else {
        Vec::new()
      },
  ...
  fn report_groups(&self, groups: &[DuplicateGroup], writer: &mut dyn io::Write, _section: ReportSection) -> io::Result<()> {
...
impl JsonReporter {
  ...
  fn write_groups(&self, groups: &[DuplicateGroup], writer: &mut dyn io::Write) -> io::Result<()> {
...
mod tests {
  ...
  fn json_relative_paths() {
    ...
    let fp = block_fingerprint();
    let group = near_group(fp, 0.9, vec![make_unit("foo", "/home/user/project/src/main.rs", 1, 10)]);
    let mut buf = Vec::new();
    reporter.report_near(&[group], &mut buf).unwrap();
    let output = String::from_utf8(buf).unwrap();
