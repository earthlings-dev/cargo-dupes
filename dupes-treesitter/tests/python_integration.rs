...
fn assert_normalized_bodies_equal(source_a: &str, source_b: &str) {
  assert_eq!(normalize_python_body(source_a), normalize_python_body(source_b));
}

fn assert_normalized_bodies_differ(source_a: &str, source_b: &str) {
  assert_ne!(normalize_python_body(source_a), normalize_python_body(source_b));
}
...
fn binary_operator_detection() {
  ...
  assert_eq!(node.children.len(), 2);
  assert_eq!(node.children[0].kind, NodeKind::Placeholder(PlaceholderKind::Variable, 0));
  assert_eq!(node.children[1].kind, NodeKind::Placeholder(PlaceholderKind::Variable, 1));
}
...
fn min_nodes_filter() {
  ...
  assert!(units.is_empty(), "Small function should be filtered out by min_nodes");
}
...
fn min_lines_filter() {
  ...
  assert!(units.is_empty(), "Single-line function should be filtered out by min_lines");
}
