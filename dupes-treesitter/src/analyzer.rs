...
impl TreeSitterAnalyzer {
  ...
  #[must_use]
  pub fn with_kind_resolver(mut self, f: impl Fn(&str) -> CodeUnitKind + Send + Sync + 'static) -> Self {
  ...
  #[must_use]
  pub fn with_test_detector(mut self, f: impl Fn(&str, tree_sitter::Node) -> bool + Send + Sync + 'static) -> Self {
...
impl LanguageAnalyzer for TreeSitterAnalyzer {
  ...
  fn parse_file(
    ...
    let tree = parser.parse(source, None).ok_or("tree-sitter failed to parse source")?;
