...
/// # Parameters
///
/// - `kind_for_node` — callback mapping the `@definition` node's tree-sitter kind string to a
///   `CodeUnitKind` (e.g., `"function_definition"` → `Function`, `"lambda"` → `Closure`)
/// - `is_test` — callback to determine whether a definition node represents test code; receives the
///   function name and the `@definition` node
///
...
pub fn extract_code_units(
  ...
  while let Some(m) = matches.next() {
    ...
    let name = name_node.and_then(|n: tree_sitter::Node| n.utf8_text(source).ok()).map_or_else(
      || {
