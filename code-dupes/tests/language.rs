...
use common::code_dupes;
use common::code_dupes_fixture_path;
...
fn assert_language_stats_error(path: &Path, language: &str, expected: &str) {
  code_dupes()
    .args(["--path", path.to_str().unwrap(), "--language", language, "stats"])
    .assert()
    .code(2)
    .stderr(predicate::str::contains(expected));
}
...
fn no_cargo_subcommand_arg_needed() {
  ...
  code_dupes()
    .args(["--path", fixture_path("exact_dupes").to_str().unwrap(), "stats"])
    .assert()
    .success();
}
...
fn auto_detect_finds_deeply_nested_rs_files() {
  ...
  std::fs::write(deep.join("lib.rs"), "pub fn deep() { println!(\"deep\"); }\n").unwrap();
  assert_stats_success(tmp.path());
}
...
fn ambiguous_language_detection_errors() {
  ...
  let tmp = tempfile::TempDir::new().unwrap();
  std::fs::write(tmp.path().join("lib.rs"), "pub fn hello() { println!(\"hello\"); }\n").unwrap();
  std::fs::write(tmp.path().join("example.py"), "def hello():\n    print('hello')\n").unwrap();
  code_dupes()
