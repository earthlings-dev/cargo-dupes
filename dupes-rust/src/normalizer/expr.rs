...
pub fn normalize_expr(expr: &syn::Expr, ctx: &mut NormalizationContext) -> NormalizedNode {
  match expr {
    ...
    // BinaryOp -> [left, right]
    syn::Expr::Binary(eb) => NormalizedNode::with_children(NodeKind::BinaryOp(normalize_bin_op(&eb.op)), vec![
      normalize_expr(&eb.left, ctx),
      normalize_expr(&eb.right, ctx),
    ]),
    // UnaryOp -> [operand]
    syn::Expr::Unary(eu) => NormalizedNode::with_children(NodeKind::UnaryOp(normalize_un_op(&eu.op)), vec![normalize_expr(&eu.expr, ctx)]),
    // Call -> [func, arg0, arg1, ...]
    syn::Expr::Call(ec) => {
    ...
    syn::Expr::Field(ef) => NormalizedNode::with_children(NodeKind::FieldAccess, vec![
    ...
    // Reference -> [expr]
    syn::Expr::Reference(er) => reference_node(er.mutability.as_ref(), PlaceholderNodeRole::Expr, &*er.expr, ctx, normalize_expr),
    ...
    // Cast -> [expr, ty]
    syn::Expr::Cast(ec) => NormalizedNode::with_children(NodeKind::Cast, vec![normalize_expr(&ec.expr, ctx), normalize_type(&ec.ty, ctx)]),
    // StructInit -> [rest_or_None, field0, field1, ...]
    syn::Expr::Struct(es) => {
      let mut children = vec![NormalizedNode::opt(es.rest.as_ref().map(|e| normalize_expr(e, ctx)))];
      children.extend(es.fields.iter().map(|f| {
        let field_idx = ctx.placeholder(&member_to_string(&f.member), PlaceholderKind::Variable);
    ...
    syn::Expr::If(ei) => NormalizedNode::with_children(NodeKind::If, vec![
    ...
    syn::Expr::Match(em) => {
      ...
      children.extend(em.arms.iter().map(|arm| {
        NormalizedNode::with_children(NodeKind::MatchArm, vec![
          ...
          NormalizedNode::opt(arm.guard.as_ref().map(|(_, g)| normalize_expr(g, ctx))),
    ...
    syn::Expr::ForLoop(ef) => NormalizedNode::with_children(NodeKind::ForLoop, vec![
    ...
    syn::Expr::Range(er) => node_with_optional_expr_pair(NodeKind::Range, er.start.as_deref(), er.end.as_deref(), ctx),
    // LetExpr -> [pat, expr]
    syn::Expr::Let(el) => {
...
pub fn normalize_stmt(stmt: &syn::Stmt, ctx: &mut NormalizationContext) -> NormalizedNode {
  match stmt {
    ...
    syn::Stmt::Local(local) => NormalizedNode::with_children(NodeKind::LetBinding, vec![
...
pub fn normalize_block(block: &syn::Block, ctx: &mut NormalizationContext) -> NormalizedNode {
  NormalizedNode::with_children(NodeKind::Block, block.stmts.iter().map(|s| normalize_stmt(s, ctx)).collect())
}
...
fn node_with_optional_expr(kind: NodeKind, expr: Option<&syn::Expr>, ctx: &mut NormalizationContext) -> NormalizedNode {
  let children = expr.map(|expr| vec![normalize_expr(expr, ctx)]).unwrap_or_default();
