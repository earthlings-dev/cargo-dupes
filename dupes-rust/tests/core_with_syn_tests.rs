...
fn fingerprint_discriminates_operators() {
  ...
  assert_ne!(Fingerprint::from_sig_and_body(&s1, &b1), Fingerprint::from_sig_and_body(&s2, &b2));
}
...
fn fingerprint_ignores_variable_names() {
  ...
  let (s1, b1) = normalize_item_fn(&f1);
  let (s2, b2) = normalize_item_fn(&f2);
  assert_eq!(Fingerprint::from_sig_and_body(&s1, &b1), Fingerprint::from_sig_and_body(&s2, &b2));
}
...
fn extracts_if_branches() {
  let body = parse_and_extract_body("fn foo(x: i32) -> i32 { if x > 0 { let y = x + 1; y * 2 } else { let z = x - 1; z * 3 } }");
  let subs = dupes_core::extractor::extract_sub_units(&body, 1);
  let if_branch_count = subs.iter().filter(|s| s.kind == CodeUnitKind::IfBranch).count();
...
fn extracts_match_arms() {
  ...
  let subs = dupes_core::extractor::extract_sub_units(&body, 1);
  let match_arm_count = subs.iter().filter(|s| s.kind == CodeUnitKind::MatchArm).count();
  assert_eq!(match_arm_count, 3);
}

#[test]
fn extracts_loop_bodies() {
  let body = parse_and_extract_body("fn foo(x: i32) { for i in 0..10 { let y = i + x; let _ = y; } }");
  let subs = dupes_core::extractor::extract_sub_units(&body, 1);
  let loop_count = subs.iter().filter(|s| s.kind == CodeUnitKind::LoopBody).count();
...
fn identical_branches_from_different_functions_match() {
  ...
  let then1 = subs1.iter().find(|s| s.description == "if-then branch").unwrap();
  let then2 = subs2.iter().find(|s| s.description == "if-then branch").unwrap();
...
fn sub_units_are_reindexed() {
  ...
  let subs = dupes_core::extractor::extract_sub_units(&body, 1);
  let then_branch = subs.iter().find(|s| s.description == "if-then branch").unwrap();

  let mut ctx = NormalizationContext::new();
  let fresh_expr = normalize_expr(&syn::parse_str::<syn::Expr>("{ let d = c + 1; d }").unwrap(), &mut ctx);
...
fn extracts_while_loop_bodies() {
  let body = parse_and_extract_body("fn foo(x: i32) { let mut i = 0; while i < x { let y = i + 1; i = y; } }");
  let subs = dupes_core::extractor::extract_sub_units(&body, 1);
  let loops: Vec<_> = subs.iter().filter(|s| s.kind == CodeUnitKind::LoopBody).collect();
...
fn extracts_bare_loop_bodies() {
  let body = parse_and_extract_body("fn foo(x: i32) -> i32 { let mut i = 0; loop { i += 1; if i > x { break i; } } }");
  let subs = dupes_core::extractor::extract_sub_units(&body, 1);
  let loops: Vec<_> = subs.iter().filter(|s| s.kind == CodeUnitKind::LoopBody).collect();
  assert_eq!(loops.len(), 1);
...
fn let_else_diverge_blocks_differ() {
  ...
  let fp1 = Fingerprint::from_sig_and_body(&sig1, &body1);
  let fp2 = Fingerprint::from_sig_and_body(&sig2, &body2);
  assert_ne!(fp1, fp2, "different let-else diverge blocks should produce different fingerprints");
}
...
fn let_else_same_diverge_blocks_match() {
  ...
  let fp1 = Fingerprint::from_sig_and_body(&sig1, &body1);
  let fp2 = Fingerprint::from_sig_and_body(&sig2, &body2);
  assert_eq!(fp1, fp2, "identical let-else structures with renamed vars should match");
}
