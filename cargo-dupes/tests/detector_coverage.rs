...
use dupes_cli_test_support::{
    assert_member_needles_absent, fixture_json, fixture_stdout, group_containing_member,
    groups_of_dimension, suppressed_count_for_rule,
};
...
const SHOW_SUPPRESSED_REPORT_ARGS: &[&str] = &[
    "--sub-function",
    "--show-suppressed",
    "--format",
    "json",
...
fn line_member_spans(report: &serde_json::Value, suffix: &str) -> Vec<(u64, u64)> {
    groups_of_dimension(report, "line")
        ...
        .map(|member| {
            (
                member["line_start"].as_u64().unwrap(),
                member["line_end"].as_u64().unwrap(),
            )
        })
        .collect()
}
...
fn suppressed_rule_attribution_pins() {
    ...
    for (rule, count) in expected {
        assert_eq!(
            suppressed_count_for_rule(&stats, rule),
            count,
            "rule {rule}"
        );
    }
...
fn show_suppressed_exposes_tagged_groups() {
    ...
    assert!(
        suppressed
            .iter()
            .all(|group| group["suppressed"].is_string()),
...
fn text_report_renders_suppression_surface() {
    ...
    let shown = fixture_stdout(
        cargo_dupes,
        FIXTURE,
        &["--sub-function", "--show-suppressed", "report"],
    );
...
fn disabling_a_rule_unhides_its_findings() {
    ...
    let arms = group_containing_member(&report, "match arm", "shapes.rs")
        .expect("dispatch arms become visible");
...
fn identical_if_chains_group_as_whole_chains() {
    ...
    let chain_fns = group_containing_member(&report, "chain_x", "overrides_a.rs")
        .expect("chain_x/chain_y function group");
    assert_eq!(chain_fns["dimension"], "ast");
    assert!(group_containing_member(&report, "chain_y", "overrides_b.rs").is_some());
    let chain_subs = group_containing_member(&report, "if chain (3 branches)", "overrides_a.rs")
        .expect("chain-level sub group");
...
fn override_branch_duplication_is_released() {
    ...
    let released = group_containing_member(&report, "if-then branch", "overrides_a.rs")
        .expect("released apply-branch group");
    ...
    let dense = groups_of_dimension(&report, "line")
        ...
        .find(|group| {
            group["members"].as_array().unwrap().iter().any(|member| {
                member["file"].as_str().unwrap().ends_with("decl_a.rs")
                    && member["line_start"].as_u64().unwrap() >= 57
            })
        })
...
fn builder_runs_are_admitted() {
    ...
    let report = fixture_json(cargo_dupes, FIXTURE, SHOW_SUPPRESSED_REPORT_ARGS);
    let builders = group_containing_member(&report, "build_one", "builders.rs")
        .expect("build_one/build_two ast near pair");
...
fn capped_boolean_projections_surface_while_trivial_shapes_stay_suppressed() {
    ...
    // rule-suppressed.
    let report = fixture_json(cargo_dupes, FIXTURE, REPORT_ARGS);
    let released = group_containing_member(&report, "ranges_overlap", "shapes.rs")
        .expect("size-cap released boolean-projection pair");
        ...
        suppressed
            ...
            .find(|group| {
                group["suppressed"] == "ast.setter-returning-self"
                    && group["members"].as_array().unwrap().iter().any(|member| {
                        member["name"].as_str().unwrap_or_default() == "Gauge::push_item"
                    })
            })
