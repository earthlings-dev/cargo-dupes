...
use dupes_cli_test_support::{
    assert_member_needles_absent, command_for_path, json_from_stdout, workspace_root,
};
...
const SELF_CORPUS_ARGS: &[&str] = &[
    ...
    "refactor",
    "--exclude",
    "cargo-dupes/tests/fixtures",
    "--exclude",
    "code-dupes/tests/fixtures",
    "--sub-function",
    "--format",
    "json",
    "report",
...
fn consolidated_sites_stay_consolidated() {
    ...
    for group in report["groups"].as_array().expect("report has groups") {
        assert!(
            !(group_has_closure_in(group, "dupes-core/src/grouper.rs")
                && group_has_closure_in(group, "dupes-core/src/lib.rs")),
...
fn group_has_closure_in(group: &serde_json::Value, file_needle: &str) -> bool {
    group["members"].as_array().is_some_and(|members| {
        members.iter().any(|member| {
            member["name"]
                .as_str()
                .is_some_and(|name| name.contains("closure"))
                && member["file"]
                    .as_str()
                    .is_some_and(|file| file.ends_with(file_needle))
        })
    })
}
    ...
    for group in report["groups"].as_array().expect("report has groups") {
        for member in group["members"].as_array().expect("group has members") {
            ...
            assert!(
                ...
                "consolidated site {relative_path}::{fn_name} (lines {start}-{end}) \
                 re-grouped: member at lines {member_start}-{member_end}"
            );
        }
    }
}
...
fn fn_span(relative_path: &str, fn_name: &str) -> (usize, usize) {
    let source = std::fs::read_to_string(workspace_root().join(relative_path))
        .expect("consolidated source file should exist");
